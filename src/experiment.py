...
def run_lora_corruption_sweep(
    ...
    """Evaluate LoRA-adapted models under image corruption across seeds.

    For each (model, category, seed, corruption, severity), loads the LoRA
    adapter trained for that (model, category, k, seed) and evaluates on the
    corresponding corrupted evaluation split. Adapters must exist at
    ``{adapter_dir}/{model}_{category}_k{k}_seed{seed}/``.

    Extending the seed dimension addresses Reviewer 2 Comment 3 (variance of
    corruption robustness across few-shot samples).

    ``seeds`` may be a single int (backward-compatible with the old signature)
    or a list. Missing adapters are logged and skipped, so partial sweeps
    remain safe.
    """
    ...
    # Backward compat: accept a single seed.
    ...
    t_start = time.time()
    n_configs = (
        len(models) * len(categories) * len(seeds)
        * len(corruption_types) * len(severity_levels)
    )
    config_idx = 0
    ...
    for model_name in models:
        for category in categories:
            ...
            for seed in seeds:
                # Recreate the same train/eval split used during LoRA training,
                # so evaluation stays on unseen images.
                train_samples, eval_samples = sample_few_shot_split(
                    samples, k=k, seed=seed,
                )

                adapter_path = (
                    adapter_dir / f"{model_name}_{category}_k{k}_seed{seed}"
                )
                if not adapter_path.is_dir():
                    print(f"[SKIP] Adapter not found: {adapter_path}")
                    # Keep progress counter accurate for the skipped block.
                    config_idx += len(corruption_types) * len(severity_levels)
                    continue

                # Load fresh classifier + trained adapter for this seed.
                classifier = load_classifier(model_name, device=device)
                    ...
                    for severity in severity_levels:
                        ...
                        try:
                            ...
                            if verbose:
                                ...
                                print(
                                    f"[{elapsed:6.1f}s] "
                                    f"[{config_idx}/{n_configs}] "
                                    f"{model_name:>6} | {category:<12} | "
                                    f"seed={seed} | "
                                    f"{corruption_type:<18} | "
                                    f"sev={severity} | "
                                    f"bal_acc={metrics['balanced_accuracy']:.3f}"
                                )
                        except Exception as e:
